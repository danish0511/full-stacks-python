...
class ArticlePublicState(SessionState):
    ...
    def get_post_detail(self):
        ...
        with rx.session() as session:
            if self.post_id == "":
                ...
                return
            sql_statement = select(BlogPostModel).options(sqlalchemy.orm.joinedload(BlogPostModel.userinfo).joinedload(UserInfo.user)).where(lookups)
    ...
    def load_posts(self, *args, **kwargs):
        ...
        with rx.session() as session:
            result = session.exec(
                select(BlogPostModel).options(sqlalchemy.orm.joinedload(BlogPostModel.userinfo)).where(lookup_args).limit(self.limit)
            ).all()
            self.posts = result
